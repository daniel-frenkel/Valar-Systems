...
String OTA_bin = "/Model_R2_1M.ino.bin"; // bin file name with a slash in front.
String ROLLBACK_bin = "/Model_R2_1M_OLD.ino.bin"; // bin file name with a slash in front.
...
void execOTA() {
  ...
  if (s3client.connect(host.c_str(), port)) {
    // Connection Succeed.
    // Fecthing the bin
    DEBUG_STREAM.println("Fetching Bin: " + String(OTA_bin));
...
void rollbackOTA() {
  ...
  if (s3client.connect(host.c_str(), port)) {
    // Connection Succeed.
    // Fecthing the bin
    DEBUG_STREAM.println("Fetching Bin: " + String(ROLLBACK_bin));
