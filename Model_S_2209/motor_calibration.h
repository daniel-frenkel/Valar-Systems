void new_auto_calibrate_step_1(){  //STEP 1 - Set Max distance 
  ...
  stepper.moveTo(-one_inch*40); //move 40 inches
  ...
  stepper.run();
        ...
        if (digitalRead(SENSOR1)==LOW && trip == 0) {
        ...
        while(stepper.currentPosition() != stepper.moveTo()){ // Wait to reach 
        stepper.run();
        }
        ...
        if (digitalRead(SENSOR1)==LOW && trip == 1) {
        ...
        if (digitalRead(SENSOR2)==LOW && trip==2) {
...
void new_auto_calibrate_step_2(){

  Serial.println("STEP 2: Setting JUST CLOSE position");
              
  position_close();

  digitalWrite(ENABLE_PIN, LOW);

  //CLOSE WINDOW to be sure window is closed
  ...
  while (!(sendData(0x35, 0) & 0x200) == 0) { //Wait until position reached flag

  sendData(0x35, 0) & 0x200;
  delay(10);
  }
              
//Now let user adjust position and press button when ready
              
  stepValue = 0;
  blynk_update = true;
  just_open_set_button=false;
              
  while(just_open_set_button==false){
  sendData(0x2D+0x80, just_open_position);
  Serial.println("Waiting for user to adjust position");
  delay(10);

  }
     
    //XTARGET 1 inch open
    sendData(0x26+0x80, MOVE_ACCEL); //DMAX
    sendData(0x27+0x80, 0); //VMAX Set velocity to 0

    Serial.print("just_open_position set: ");
    Serial.println(just_open_position);
    preferences_local.putInt("just_open", just_open_position);


    Serial.println("just_open_position set");
...
while (number_open_passes<=current_high_open_passes || number_close_passes<=current_high_open_passes) {
  ...
  if (!digitalRead(SENSOR1) && calibration_direction==1) {
...
while (number_open_passes<=current_low_open_passes || number_close_passes<=current_low_close_passes) {
    ...
    if (!digitalRead(SENSOR1) &&  calibration_direction == 1 && run_section == 1) { //IF POS 1 reched
    Serial.println("Position 1 Tripped!");
    sendData(0x26+0x80, MOVE_DECEL); //Decelerate super fast
    sendData(0x27+0x80, 0); //Set velocity to 0
    delay(200);
  ...
  if (!digitalRead(SENSOR2) &&  calibration_direction == 1 && run_section == 2) { //Try not to hit this
...
void position_adjust(){
     ...
     //sendData(0x21+0x80, 0);      // XACTUAL=0 to start motion
              ...
              while(trip<1){
                  ...
                  if ((digitalRead(SENSOR2)==LOW) || millis()>max_time) {
     ...
     sendData(0x21+0x80, 0);      // XACTUAL=0 to start motion
              ...
              while(trip<1){
                  ...
                  if ((digitalRead(SENSOR1)==LOW) || millis()>max_time) {
                      sendData(0x26+0x80, MOVE_DECEL); //DMAX
                      sendData(0x27+0x80, 0); //VMAX Set velocity to 0
                      delay(500);
                      trip=1;
